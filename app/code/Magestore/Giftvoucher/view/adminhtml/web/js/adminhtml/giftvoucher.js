...
require([
    ...
    sendFriend = function(el) {
        ...
        if (el.checked) {
            if (receiver) {
                receiver.show();
                JQuery('.giftvoucher-receiver').show();
                if ($('recipient_name'))
...
//    }
    ...
    validateInputRange = function(el, from, to, priceFormat, gift_price_type, gift_price) {
        ...
        if (gift_price_type == '3') {
            ...
            jQuery("#product_composite_configure_input_qty").attr('price', newPrice);
        } else {
    ...
    setAmountDropDown = function(el, gift_price_type, gift_price) {
        if (gift_price_type == '3') {
            var newPrice = el.value * gift_price / 100;
            var input_hidden = '<input id="abc" type="hidden" value="1" price="'+newPrice+'" qtyid="product_composite_configure_input_qty">';
            ...
            if (priceArray[el.selectedIndex]) {
                var input_hidden = '<input id="abc" type="hidden" value="1" price="'+priceArray[el.selectedIndex]+'" qtyid="product_composite_configure_input_qty">';
            }
        } else {
